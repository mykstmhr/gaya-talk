...
gaya-talk の「ルーム」機能のための WebSocket リレーサーバ(Cloudflare Workers + Durable Objects)。
...
設定後は、ルームを作成する人だけがクライアント側の config(`room.create_secret` または環境変数 `GAYATALK_ROOM_CREATE_SECRET`)に同じ値を設定する。参加だけのメンバーには不要。
