...
会議や発表の本線を邪魔せずに、ガヤ・実況コメントを流す macOS 常駐ツール。声やキー入力をその場で文字にして、画面全体の透過オーバーレイに右から左へ流す。同じルームに参加したメンバーの画面にも同じコメントが流れる。

- どのビデオツールの上でも動き、**画面共有・収録には映らない**(クリックも下のアプリへ素通し)
