...
**匿名 / 記名**: 既定の匿名ルームではコメントに名前は付かない(起動ごとのランダムな色で同一人物を追える)。「記名」で作ったルームでは各コメントに `[表示名]` が付く。表示名は `room.display_name`、未設定なら**記名ルームの作成/参加時に入力を求められる**(入力した名前は内部ファイルに保存され次回以降は聞かれない)。匿名/記名はルーム作成者が決め、共有 URL に含まれるので参加者全員で揃う(URL の `&n=1` が記名)。
