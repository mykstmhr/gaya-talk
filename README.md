...
**匿名 / 記名**: 既定の匿名ルームではコメントに名前は付かない(起動ごとのランダムな色で同一人物を追える)。「記名」で作ったルームでは各コメントに `[表示名]` が付く。表示名は `room.display_name`、空なら macOS のユーザー名。匿名/記名はルーム作成者が決め、共有 URL に含まれるので参加者全員で揃う(URL の `&n=1` が記名)。
