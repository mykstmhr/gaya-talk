...
イメージは「**番組のガヤ / ライブの実況コメント**」。Gather / Google Meet / Zoom などの会議や発表で、音声で割り込むと流れの腰を折ってしまう場面でも、相槌・ツッコミ・短いコメントを声やタイプで流して場を盛り上げられる。ビデオツール非依存で、どのアプリの上にも流れる(クリックは下のアプリへ素通し)。**会議の画面共有・収録には映らない**ので、ガヤが本線の相手に見えることはない。
...
起動中のバージョンはメニューバーのメニュー下部に表示される(例「バージョン : v0.2.5(リリース版)」。自分でビルドしたものは「ローカルビルド」)。リリース版なら「**アップデートを確認…**」から最新版の確認と更新(ダウンロード → 差し替え → 再起動)がそのままできる。
...
- **再起動・アップデート後は前回のルームに自動で入り直す**(メニューの「ルームから退出」で退出した場合は覚えない)
- **画面共有には映らない**ので、Meet で画面共有していてもコメントは相手に見えない
