...
**作成認証(推奨)**: サーバ URL は招待 URL の一部として全参加者に見えるため、既定では URL を知る誰でもルームを作れてしまう。`(cd server && npx wrangler secret put CREATE_SECRET)` で任意のシークレットを設定すると、ルーム作成に認証が掛かる。設定したら、あなた(ルームを作る人)の config の **`room.create_secret`**(または環境変数 **`URATALK_ROOM_CREATE_SECRET`**)に同じ値を書く。参加するだけのメンバーには不要。
...
アプリ本体のアンインストールは [README](../README.md#アンインストール) を参照。ホストはそれに加えて:

```sh
(cd server && npx wrangler delete)   # Worker と Durable Objects(全ルームのメタデータ)を削除
```

削除すると**全ルームの URL が即座に無効になる**(メンバーへ事前に知らせること)。Slack ミラーを設定していた場合は、Slack 側のアプリ管理画面から作成した Slack アプリも削除(またはトークンを revoke)しておく。
