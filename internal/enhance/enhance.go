...
func (e *Enhancer) EnsureServer(ctx context.Context) (started bool, err error) {
	...
	if e.reachable(ctx) {
		// 専用ポートで既に動いているものは前回(クラッシュ等)の gaya-talk の残骸なので、
		// 引き取って終了時に止める対象にする(専用ポートは gaya-talk しか使わない前提)。
...
// 専用ポート(既定 11477 等)のとき: gaya-talk 専用インスタンスとして OLLAMA_HOST を
...
func (e *Enhancer) startServer() error {
	...
	// どちらの場合も gaya-talk のプロセスグループから切り離す(親の SIGKILL に巻き込まれて
