...
type VoiceConfig struct {
	...
	// 切り替わって再生音が途切れるのを防げる。`gaya-talk devices` で候補を確認できる。
...
type RoomConfig struct {
	// Server は中継サーバ(Cloudflare Workers)の URL。例 "https://gaya-talk-room.<name>.workers.dev"。
	// 空でもオーバーレイ自体は動く(ルーム未参加=自分の画面にだけ流れるソロモード)。
	Server string `json:"server"`
	// CreateSecret はルーム作成時の認証シークレット。サーバ側で CREATE_SECRET を設定して
	// いる場合に必要(参加だけなら不要)。環境変数 GAYATALK_ROOM_CREATE_SECRET でも渡せる。
	...
	// 環境変数 GAYATALK_SLACK_BOT_TOKEN でも渡せる(config に平文で書きたくない場合)。
...
// Load は設定を読み込む。検索順は環境変数 GAYATALK_CONFIG → ./config.json →
// ~/.config/gaya-talk/config.json。見つからない項目はデフォルトを使う。
func Load() (*Config, error) {
	...
	if v := os.Getenv("GAYATALK_ROOM_SERVER"); v != "" {
		cfg.Room.Server = v
	}
	if v := os.Getenv("GAYATALK_WHISPER_MODEL"); v != "" {
		cfg.Whisper.Model = v
	}
	if v := os.Getenv("GAYATALK_SLACK_BOT_TOKEN"); v != "" {
		cfg.Room.SlackBotToken = v
	}
	if v := os.Getenv("GAYATALK_ROOM_CREATE_SECRET"); v != "" {
...
// 検索順: 環境変数 GAYATALK_CONFIG → ./config.json → ~/.config/gaya-talk/config.json。
func Path() string {
	if p := os.Getenv("GAYATALK_CONFIG"); p != "" {
		return p
	}
	...
	if home, err := os.UserHomeDir(); err == nil {
		return filepath.Join(home, ".config", "gaya-talk", "config.json")
	}
	return ""
}
